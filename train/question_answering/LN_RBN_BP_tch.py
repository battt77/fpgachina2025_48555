...
class Function(torch.autograd.Function):
  ...
  def forward(ctx,x,weight,bias,normalized_shape=12,eps=1e-12,batch_size=2):
      ...
      bottom_dims=int(normalized_shape/batch_size)
      x_c=torch.reshape(x,shape=input_shape[:-1]+[batch_size,bottom_dims])
      c_x_shape=x_c.shape
      batch_size_tensor=torch.tensor(batch_size, dtype=x.dtype, device=x.device)
      C=torch.rsqrt(2*torch.log(batch_size_tensor))

      max = torch.max(x_c,dim=-1,keepdim=True).values
      min = torch.min(x_c,dim=-1,keepdim=True).values

      #max_pos=torch.max(x_c,dim=-1,keepdim=True).indices     #find the max and min in order to calcuating their gradient and create onehot
      #min_pos=torch.min(x_c,dim=-1,keepdim=True).indices
      ...
      range_var_div = range_var_sum/(2*batch_size_tensor)
      ...
      #
      normalized_shape_tensor=torch.tensor(normalized_shape, dtype=x.dtype, device=x.device)
      eps_tensor=torch.tensor(eps, dtype=x.dtype, device=x.device)
      c_x_shape_tensor=torch.tensor(list(c_x_shape), dtype=torch.int64, device=x.device)
      ctx.save_for_backward(normalized_shape_tensor,norm,weight,bias,variance,eps_tensor,batch_size_tensor,max_pos,min_pos,C,c_x_shape_tensor)
      return result
  ...
  def backward(ctx,grad_output):
        ...
        gamma = weight
        norm=norm
        c_x_shape=[int(s) for s in c_x_shape.tolist()]
