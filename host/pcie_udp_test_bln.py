"""
LayerNorm Acceleration Circuit Evaluation
...
def send_all_data_via_pcie():
    ...
    try:
        ...
        print(f"  Shape: ({X_DIM}, {Y_DIM}), Address: 0x{ADDR_X_DATA:08X}")
        ...
        print(f"  Shape: ({X_DIM}, {Y_DIM}), Address: 0x{ADDR_WEIGHTS:08X}")
        ...
        np.random.seed(456)
        bias_single = np.random.uniform(-0.2, 0.2, Y_DIM).astype(np.float16)
        bias = np.tile(bias_single, X_DIM)
        print(f"  Shape: ({X_DIM}, {Y_DIM}), Address: 0x{ADDR_BIAS:08X}")
...
def read_and_compare_results(reference_output, ref_delta_x, ref_delta_weight, ref_delta_bias, cpu_fw_time, cpu_bw_time, iteration=None, total_iterations=None):
    ...
    try:
        ...
        print(f"[2/5] Reading input gradient...")
        ...
        print(f"[3/5] Reading weight gradient...")
        ...
        # ====== 4. Read bias gradient ======
        print(f"[4/5] Reading bias gradient...")
        grad_bias_size = Y_DIM * 2
        fpga_grad_bias_bytes = c2h_transfer(ADDR_GRAD_BIAS, grad_bias_size)
        
        if fpga_grad_bias_bytes is None:
            print("❌ Failed to read bias gradient")
            return False
        
        fpga_grad_bias = np.frombuffer(fpga_grad_bias_bytes, dtype=np.float16)
        fpga_grad_bias_tensor = torch.from_numpy(fpga_grad_bias)
        
        bias_is_close = torch.allclose(ref_delta_bias.cpu().detach(), fpga_grad_bias_tensor, rtol=RTOL, atol=ATOL)
        diff_bias = torch.abs(ref_delta_bias.cpu().detach() - fpga_grad_bias_tensor)
        
        if bias_is_close:
            print(f"  ✓ Bias gradient verified (rtol={RTOL}, atol={ATOL})")
        else:
            print(f"  ❌ Bias gradient mismatch (max_diff: {torch.max(diff_bias).item():.6f})")
        
        # ====== 5. Read FPGA timing ======
        print(f"[5/5] Reading FPGA timing...")
        clk_size = 12
        ...
        if fpga_clk_bytes is not None and len(fpga_clk_bytes) == clk_size:
            ...
            bm_param_cycles = struct.unpack('<I', fpga_clk_bytes[4:8])[0]  # weight+bias gradient backprop
            ...
            if not iteration:
                print(f"\n{'='*70}")
                print(f"{'LayerNorm Acceleration Circuit Evaluation':^70}")
                print(f"{'='*70}")
                ...
                print(f"  Bias Gradient     : {'PASS' if bias_is_close else 'FAIL':>8}  (max_diff: {torch.max(diff_bias).item():.6f})")
                ...
                # Note: FPGA Backward includes both weight/bias gradient and input gradient backprop
                print(f"\nNote: FPGA Backward = Weight+Bias Grad ({bm_param_time_ms:.3f}ms) + Input Grad ({bm_time_ms:.3f}ms)")
            else:
                # In multi-iteration mode, just show brief status
                print(f"  ✓ Timing recorded (FW: {fm_time_ms:.5f}ms, BW: {fpga_bw_time_ms:.5f}ms)")
        
        # Summary
        all_match = fw_is_close and grad_x_is_close and weight_is_close and bias_is_close
...
def main():
    """Main function: PCIe + UDP workflow"""
    NUM_ITERATIONS = 10  # Number of test iterations
    
    print("\n" + "="*70)
    print("LayerNorm FPGA Acceleration Test (Config: {}x{}, Batch={})".format(X_DIM, Y_DIM, BATCH_SIZE))
    ...
    try:
        ...
        if len(cpu_fw_times) > 0 and len(fpga_fw_times) > 0:
            print(f"\n\n{'='*70}")
            print(f"{'LayerNorm Acceleration Circuit - Average Results':^70}")
            print(f"{'='*70}")
            ...
            bias_pass_rate = sum(1 for acc in accuracy_results if acc['bias']) / len(accuracy_results) * 100
            ...
            print(f"  Bias Gradient     : {bias_pass_rate:>6.1f}% pass rate")
            ...
            print(f"\nNote: Backward time = Weight+Bias Gradient + Input Gradient")
